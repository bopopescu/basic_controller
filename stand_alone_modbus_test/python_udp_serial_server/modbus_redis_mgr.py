...
class ModbusRedisServer():  # manages the local functions of io server
   def __init__(self, message_handler, redis_db ):
       ...
       self.ping_functions        = {"ping_device": self.ping_device_a,"ping_all_devices":   self.ping_all_devices_a }
   ...
   def counter_functions( self, json_object ):
      function       = json_object["action"]
      parameter      = json_object["parameters"]
      if self.counter_functions.has_key(function):
          return self.counter_functions[ funtion ](sub_parameter)
      else:
          return None
                          
   def clear_counter_list( self , par ):
       for i in parameters:
   ...
   def ping_functions( self, json_object ):
      ...
      return self.ping_functions[ function ](parameters)    
